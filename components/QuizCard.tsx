...
const QuizCard: React.FC<QuizCardProps> = ({ question, onAnswer, selectedOptionId, isAnswered }) => {
  ...
  return (
    <div className="w-full">
      <div className="bg-white/70 backdrop-blur-md rounded-[2rem] shadow-xl shadow-indigo-100/40 p-8 mb-6 text-center border border-white/60 relative overflow-hidden group transition-all duration-300">
        ...
        <div className="relative z-10">
            ...
            <h2 className="text-5xl font-black text-gray-800 mb-6 min-h-[4rem] flex items-center justify-center break-keep tracking-tight drop-shadow-sm leading-tight transition-all duration-200">
            {getQuestionText()}
            </h2>
      ...
      <div className="grid grid-cols-1 gap-3">
        {options.map((option, idx) => {
          ...
          return (
            <div key={option.id} className="animate-slide-in" style={{ animationDelay: `${idx * 30}ms` }}>
                <Button
                variant={variant}
                className={`relative flex items-center justify-center min-h-[4.5rem] text-lg transition-all duration-200 ${
                ...
                {isAnswered && isCorrect && <CheckCircle2 className="absolute right-5 w-6 h-6 animate-in zoom-in duration-200 text-white z-10" />}
                {isAnswered && isSelected && !isCorrect && <XCircle className="absolute right-5 w-6 h-6 animate-in zoom-in duration-200 text-white z-10" />}
                </Button>
            </div>
          );
        })}
      ...
      {isAnswered && (
        <div className="mt-6 p-6 bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-lg shadow-indigo-100/50 border border-white/60 text-left animate-fade-in overflow-hidden relative duration-200">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 bg-gray-50/50 -mx-6 -mb-6 p-6 min-h-[6rem]">
                ...
                {isLoadingExample ? (
                    ...
                    <p className="text-gray-700 text-base leading-relaxed font-medium animate-in fade-in duration-300">"{example}"</p>
                ) : (
